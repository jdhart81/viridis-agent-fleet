...
|---|---|---|---|
| Smithery listings owned by `hartjustin6` | **Allowed.** Smithery documents an owner-authenticated `PATCH /servers/{qualifiedName}` API specifically for updating display metadata including description and homepage. | Cleared for autonomous metadata refresh | Dedicated Smithery API key; adapter hard-restricts the namespace to `hartjustin6/*`; 30-day per-listing cooldown; no creation or modification of another owner's listing. |
