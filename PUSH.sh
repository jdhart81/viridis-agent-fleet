...
[ -d .git ] || git init -q
git add -A
git commit -q -m "MCP surface: 23-mount Viridis A2A economy — manifests, tool schemas, gateway (v0.2.0 usage stats + Verified relay), contracts, x402-C draft" || echo "(nothing to commit)"
git branch -M main
