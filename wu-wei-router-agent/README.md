...
Inspect `service-schema.json` and `example.json`; an unsigned POST returns an unpaid quote. `describe_agent` is free. Buyer-authorized payment is required for execution through the fleet. The standalone core is a computation library and must not be exposed outside the fleet payment gate.
