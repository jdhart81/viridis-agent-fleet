...
HUB_KERNEL_PRESENT = (GATEWAY / "hub_kernel.py").is_file()
...
def security_receipt(private, issuer_id, target_agent_id, *,
                     ...
                     scanner_name="viridis-injection-detector",
                     scanner_version="0.1.0", profile_sha256="",
                     result_counts=None):
    coverage = ["supplied-prompt-input", "tool-injection"]
    if scanner_name == "Viridis Security Preflight":
        coverage = [
            "manifest", "tool-schemas", "tool-policy",
            "artifact-sha256:" + hashlib.sha256(
                f"artifact:{target_agent_id}:{suffix}".encode()).hexdigest(),
        ]
        if profile_sha256:
            coverage.append("profile-sha256:" + profile_sha256)
    unsigned = {
...
def test_signed_security_attestation_is_expiring_evidence_not_a_guarantee(core):
    ...
    assert posture["coverage_score"] == 0.3333
...
def test_preflight_v11_receipt_is_profile_bound_and_stales_after_change(tmp_path):
    issuer_private, issuer_public = keys()
    item = MarketNetworkCore(
        db_path=str(tmp_path / "preflight-bound.sqlite3"), now_fn=lambda: NOW,
        trusted_security_receipt_keys={
            "viridis-security-preflight": issuer_public})
    profiles = [
        {"agent_id": "viridis-security-preflight", "name": "Preflight",
         "description": "Operator-owned security preflight receipt issuer.",
         "capabilities": ["security"], "representative_queries": [],
         "endpoint": "https://mcp.viridisconservation.com/security-preflight/mcp",
         "payment": {}, "operator_entity": "ViridisNorth LLC"},
        {"agent_id": "bound-target", "name": "Bound Target",
         "description": "Original exact target profile for binding validation.",
         "capabilities": ["target"], "representative_queries": [],
         "endpoint": "https://agents.example.com/bound-target/mcp",
         "payment": {}, "operator_entity": "ViridisNorth LLC"},
    ]
    try:
        item.seed_owned_profiles(profiles)
        original_sha = item._profile_row("bound-target")["profile_sha256"]
        receipt, signature = security_receipt(
            issuer_private, "viridis-security-preflight", "bound-target",
            scanner_name="Viridis Security Preflight",
            scanner_version="1.1.0", profile_sha256=original_sha,
            suffix="bound")
        imported = run(item.process({
            "action": "import_security_receipt",
            "receipt": receipt,
            "signature_b64": signature,
        }))
        assert imported["status"] == "ok", imported
        current = item._security_posture("bound-target")
        assert current["coverage_score"] == 0.5
        assert current["ranking_eligible_attestations"] == 1

        changed = [dict(profile) for profile in profiles]
        changed[1]["description"] = (
            "Changed target profile whose old security evidence must go stale.")
        assert item.seed_owned_profiles(changed) == 1
        stale = item._security_posture("bound-target")
        assert stale["status"] == "UNASSESSED"
        assert stale["current_attestations"] == 0
        assert stale["reported_current_attestations"] == 1
        assert stale["stale_profile_bound_attestations"] == 1
        assert stale["coverage_score"] == 0
    finally:
        item.close()


def test_preflight_v11_unbound_or_wrong_profile_receipt_fails_closed(tmp_path):
    issuer_private, issuer_public = keys()
    item = MarketNetworkCore(
        db_path=str(tmp_path / "preflight-reject.sqlite3"), now_fn=lambda: NOW,
        trusted_security_receipt_keys={
            "viridis-security-preflight": issuer_public})
    try:
        item.seed_owned_profiles([
            {"agent_id": "viridis-security-preflight", "name": "Preflight",
             ...
             "payment": {}},
            {"agent_id": "receipt-target", "name": "Receipt Target",
             "description": "Target for fail-closed profile binding tests.",
             "capabilities": ["target"], "representative_queries": [],
             "endpoint": "https://agents.example.com/receipt-target/mcp",
             "payment": {}},
        ])
        unbound, unbound_signature = security_receipt(
            issuer_private, "viridis-security-preflight", "receipt-target",
            scanner_name="Viridis Security Preflight",
            scanner_version="1.1.0", suffix="unbound")
        rejected = run(item.process({
            "action": "import_security_receipt",
            "receipt": unbound,
            "signature_b64": unbound_signature,
        }))
        assert rejected["status"] == "error"
        assert rejected["field"] == "receipt.coverage"

        wrong, wrong_signature = security_receipt(
            issuer_private, "viridis-security-preflight", "receipt-target",
            scanner_name="Viridis Security Preflight",
            scanner_version="1.1.0", profile_sha256="a" * 64,
            suffix="wrong")
        rejected = run(item.process({
            "action": "import_security_receipt",
            "receipt": wrong,
...
def test_failing_security_result_receives_no_ranking_credit(tmp_path):
    issuer_private, issuer_public = keys()
    item = MarketNetworkCore(
        db_path=str(tmp_path / "failing-security.sqlite3"), now_fn=lambda: NOW,
        trusted_security_receipt_keys={"scanner-agent": issuer_public})
    try:
        item.seed_owned_profiles([
            {"agent_id": "scanner-agent", "name": "Scanner Agent",
             "description": "Security result receipt issuer.",
             "capabilities": ["security"], "representative_queries": [],
             "endpoint": "https://scanner.example.com/mcp", "payment": {}},
            {"agent_id": "finding-target", "name": "Finding Target",
             "description": "Target with a reported security finding.",
             "capabilities": ["target"], "representative_queries": [],
             "endpoint": "https://agents.example.com/finding-target/mcp",
             "payment": {}},
        ])
        receipt, signature = security_receipt(
            issuer_private, "scanner-agent", "finding-target",
            result_counts={
                "checks": 4, "passed": 3, "warnings": 0,
                "findings": 1, "errors": 0,
            },
            suffix="finding")
        imported = run(item.process({
        ...
        assert imported["status"] == "ok", imported
        posture = item._security_posture("finding-target")
        assert posture["status"] == "COVERAGE_REPORTED"
        assert posture["coverage_score"] == 0
        assert posture["ranking_eligible_attestations"] == 0
    finally:
        item.close()
...
@pytest.mark.skipif(not HUB_KERNEL_PRESENT, reason="private Hub Kernel is absent")
def test_market_funding_event_composes_with_real_hub_verifier(tmp_path):
    HubKernel = importlib.import_module("hub_kernel").HubKernel
    class Store:
...
def test_unverified_control_keeps_usefulness_counters_separate_and_fail_closed(
        core):
    buyer_key, seller_key, work, _, _ = full_awarded(core)
    core.hub_required = True
    core._settlement_verifier = lambda event: {
    ...
    assert core.network_status()["operator_verification_trusted_issuers"] == []
    result = usefulness(
        core, "buyer-agent", buyer_key, work["work_id"],
        outcome="USEFUL", would_buy_again=True, suffix="unverified-control")
    assert result["status"] == "ok"
    assert result["data"]["buyer_seller_relation"] == (
        "CONTROL_RELATION_UNVERIFIED")
    assert result["data"]["independent_buyer"] is False
    ...
    assert status["independently_useful_paid_deliveries"] == 0
    assert status["would_buy_again_count"] == 1
    assert status["operator_verification_receipts_imported"] == 0
    assert status["operator_verifications_current"] == 0


def test_self_deal_is_refused_before_offer_and_cannot_create_usefulness(core):
    private, _ = register(
        core, "self-dealing-agent", "procurement", "One Operator LLC")
    work = post_work(core, "self-dealing-agent", private)
    body = {
        "work_id": work["work_id"], "amount_minor": 400, "currency": "USD",
        "proposal": "An agent must not buy its own work.",
        "delivery_seconds": 3600,
        "settlement": {
            "rail": "x402",
            "payment_endpoint":
                "https://agents.example.com/self-dealing-agent/x402/run",
            "network": "eip155:8453", "asset": "USDC",
        },
        "idempotency_key": "self-offer-submit-0001",
    }
    refused = run(core.process(signed_input(
        "submit_offer", "seller_id", "self-dealing-agent", private, body,
        "self-offer-submit-nonce-0001")))
    assert refused["status"] == "error"
    assert refused["field"] == "seller_id"
    status = core.network_status()
    assert status["buyer_feedback_jobs"] == 0
    assert status["buyer_signed_useful_paid_deliveries"] == 0
    assert status["independently_useful_paid_deliveries"] == 0
...
def test_operator_verification_expiry_removes_independent_usefulness(tmp_path):
    clock = [NOW]
    core = MarketNetworkCore(
        db_path=str(tmp_path / "operator-usefulness-expiry.sqlite3"),
        now_fn=lambda: clock[0])
    try:
        buyer_key, seller_key, work, _, _ = full_awarded(core)
        core.hub_required = True
        ...
        accepted = usefulness(
            core, "buyer-agent", buyer_key, work["work_id"], suffix="expiry")
        assert accepted["data"]["independent_buyer"] is True
        assert core.network_status()[
            "independently_useful_paid_deliveries"] == 1
        ...
        assert status["buyer_signed_useful_paid_deliveries"] == 1
        assert status["independently_useful_paid_deliveries"] == 0
        feedback = core.get_work(work["work_id"])["buyer_feedback"]
        assert feedback["independent_buyer"] is False
        assert feedback["buyer_seller_relation"] == (
            "VERIFICATION_EXPIRED_OR_INVALIDATED")
    finally:
        core.close()


def test_profile_change_removes_existing_independent_usefulness(core):
    buyer_key, seller_key, work, _, _ = full_awarded(core)
    core.hub_required = True
    core._settlement_verifier = lambda event: {
        "verified": True, "event_id": event["event_id"],
    ...
    accepted = usefulness(
        core, "buyer-agent", buyer_key, work["work_id"], suffix="profile-change")
    assert accepted["data"]["independent_buyer"] is True
    assert core.network_status()["independently_useful_paid_deliveries"] == 1

    buyer_public = b64(buyer_key.public_key().public_bytes(
        serialization.Encoding.Raw, serialization.PublicFormat.Raw))
    changed = profile_payload(
        "buyer-agent", buyer_key, buyer_public, capability="procurement",
        idem="buyer-profile-change-0002",
        nonce="buyer-profile-change-nonce-0002",
        operator_entity="Independent Buyer LLC")
    changed["description"] = (
        "Updated deterministic procurement agent for autonomous buyers.")
    changed_body = {
        key: value for key, value in changed.items()
        if key not in {"action", "agent_id", "auth"}
    }
    changed["auth"] = auth(
        buyer_key, "publish_profile", "buyer-agent", changed_body,
        "buyer-profile-change-nonce-0002")
    assert run(core.process(changed))["status"] == "ok"
...
def test_energyai_seed_profile_exposes_services_and_evidence_boundaries():
    ...
    assert "source-matched installer quote terms" in energyai["description"]
    assert "source-matched-quote-review" in energyai["capabilities"]
    assert "operator-value-receipts" in energyai["capabilities"]
    assert "energy-workflow-discovery" in energyai["capabilities"]
    assert "$19/month with $20 monthly tool credit" in energyai["description"]
    assert "not vetted, endorsed, or partnered" in energyai["description"]
    assert "Human review is optional and separate" in energyai["description"]
    assert "Quote results establish execution only" in energyai["description"]
    assert "paid repeat use need separate evidence" in energyai["description"]
    assert "non-cash tool credit, not a cash payout" in energyai["description"]
    assert "Find publicly-rated clean-energy contractors near a US ZIP code" in (
        energyai["representative_queries"])
...
def test_reliability_sprint_repositions_existing_profile_without_new_agent():
    payload = json.loads(
        (Path(__file__).parents[1] / "seed_profiles.json").read_text())
    delivery = next(
        p for p in payload["profiles"]
        if p["agent_id"] == "viridis-mcp-delivery")

    assert delivery["name"] == "Viridis Agent Reliability Sprint"
    assert delivery["endpoint"] == (
        "https://mcp.viridisconservation.com/network/mcp")
    assert set(delivery["capabilities"]) == {
        "agent-reliability", "ai-automation", "api-integration",
        "deployment-testing", "human-in-the-loop", "mcp",
        "observability", "production-readiness", "security-preflight",
        "workflow-automation",
    }
    for boundary in (
            "Five-business-day production-readiness service",
            "one existing AI-agent or automation workflow",
            "up to two existing systems or APIs",
            "human approval gates for irreversible actions",
            "security and tool-policy preflight",
            "automated tests",
            "deployable handoff",
            "verifiable delivery receipt",
            "Introductory list price: $995 USD",
            "indefinite maintenance are excluded",
            "does not begin before",
            "independently verifies exact live cash-escrow funding",
            "not included, active, or automatically enrolled"):
        assert boundary in delivery["description"]
    assert delivery["payment"] == {
        "cash_escrow_endpoint":
            "https://mcp.viridisconservation.com/payments/mcp",
        "payee_id": "viridis:mcp-delivery",
        "price_minor": 99500,
        "currency": "USD",
    }
    assert "x402_endpoint" not in delivery["payment"]
    assert "public_key_b64" not in delivery


def test_container_runtime_dependencies_are_exactly_pinned():
    requirements = (
    ...
    lock_pins = {
        ...
        for line in lock if line and not line.startswith("#")
    }
    for pin in pins:
        package = pin.split("==", 1)[0].casefold()
        assert lock_pins[package] == pin
    dockerfile = (
        Path(__file__).parents[1] / "Dockerfile"
    ).read_text()
    assert "COPY agent-market-network-agent/requirements.lock" in dockerfile
    assert "pip install --no-cache-dir -r requirements.lock" in dockerfile


def test_mcp_delivery_profile_can_sign_one_custom_cash_offer(core):
    payload = json.loads(
        (Path(__file__).parents[1] / "seed_profiles.json").read_text())
    delivery = next(
        p for p in payload["profiles"]
        if p["agent_id"] == "viridis-mcp-delivery")
    seller_private, seller_public = keys()
    core.seed_owned_profiles(
        [delivery], write_public_keys={
            "viridis-mcp-delivery": seller_public})

    profile = core.search_agents(
        "production reliable MCP workflow with approvals and logging",
        capabilities=["mcp", "production-readiness"],
        payment_rail="viridis_cash_escrow")["results"][0]
    assert profile["agent_id"] == "viridis-mcp-delivery"
    assert profile["auth_mode"] == "operator_signed_ed25519"
    assert profile["payment"]["payee_id"] == "viridis:mcp-delivery"
    assert profile["payment"]["price_minor"] == 99500

    buyer_private, _ = register(
        core, "external-delivery-buyer", "procurement",
        operator_entity="Independent Buyer LLC")
    work = post_work(
        core, "external-delivery-buyer", buyer_private,
        idem="delivery-sprint-work", nonce="delivery-sprint-work-nonce")
    bid = cash_offer(
        core, "viridis-mcp-delivery", seller_private, work["work_id"],
        idem="delivery-sprint-offer", nonce="delivery-sprint-offer-nonce",
        payee_id="viridis:mcp-delivery")

    assert bid["amount_minor"] == 400
    assert bid["currency"] == "USD"
    assert bid["settlement"] == {
        "rail": "viridis_cash_escrow",
        "payment_endpoint":
            "https://mcp.viridisconservation.com/payments/mcp",
        "payee_id": "viridis:mcp-delivery",
        "release_rail": (
            "Stripe Connect when payee is onboarded; certified manual "
            "fallback otherwise"),
    }
