"""Pin the fleet-standard non-dict input guard (N54 idiom) — Night 62.

process() must never raise on non-dict input; it must return the canonical
A2A structured error envelope. Existing suites asserted only status=="error"
(shallow pin) or nothing at all; this pins the full envelope shape
