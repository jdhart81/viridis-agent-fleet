...
spec = importlib.util.spec_from_file_location(
    "watch_security_core", HERE.parents[1] / "security-preflight-agent/src/core.py")
...
def test_gateway_routes_discovery_and_real_public_receipt(core, monkeypatch, tmp_path):
    import viridis_mcp_gateway as gateway
    monkeypatch.setenv("STATE_DB", str(tmp_path / "gateway.db"))
    monkeypatch.setenv("SECURITY_PREFLIGHT_RECEIPT_DB_PATH", core._receipt_db_path)
    ...
    with TestClient(gateway.build_app()) as client:
        result = client.post(watch.PATH, json={"inputs": inputs(), "baseline_receipt_id": receipt_id})
        assert result.status_code == 200, result.text
        assert result.json()["decision"] == "UNCHANGED"
        ...
        assert "sample_inputs" not in public.json()["evidence"]
        assert watch.PATH in client.get("/openapi.json").json()["paths"]
        catalog = client.get("/x402/catalog").json()
        assert catalog["adoption"]["security_change_check"]["version"] == watch.VERSION
...
def test_paid_result_carries_the_exact_same_route_and_recheck_baseline(core):
    import x402_http
    result = scan(core, inputs())
    enriched = x402_http._with_commerce_metadata(result, "security-preflight", "security_preflight", "https://mcp.test")
    loop = enriched["viridis_commerce"]
    assert loop["repeat_purchase"]["tool"] == "security_preflight"
    assert loop["change_check"]["baseline_receipt_id"] == result["receipt"]["receipt_id"]
    assert loop["change_check"]["endpoint"].endswith(watch.PATH)
