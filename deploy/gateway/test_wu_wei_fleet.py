...
ROUTE = ("wu-wei-router", "plan_workload")
...
def setup(monkeypatch, tmp_path):
    ...
    spec = importlib.util.spec_from_file_location("wu_wei_paid_fixture", ROOT / "wu-wei-router-agent/src/core.py")
    module = importlib.util.module_from_spec(spec); spec.loader.exec_module(module)
    core = module.WuWeiRouterCore()
...
import os
enabled = os.environ.get('WU_WEI_FLEET_ENABLED') == '1'
assert ('wu-wei-router' in g.MOUNTS) == enabled
assert (('wu-wei-router','plan_workload') in x.X402_HTTP_TOOLS) == enabled
...
if enabled:
    ...
    assert 'wu-wei-router' in str(entries)
    g._load_adapter('maxwell-defense', 'maxwell-defense-agent')
    module = g._load_adapter('wu-wei-router', 'wu-wei-router-agent')
    ...
    with patch.object(module.agent, 'process', side_effect=AssertionError('invalid input reached payment gate')):
        refusal = json.loads(asyncio.run(module.plan_workload([], [])))
        assert refusal['error_type'] == 'ValidationError'
"""
    for flag in ("0", "1"):
        result = subprocess.run([sys.executable, "-c", code], cwd=Path(__file__).parent,
                                env=os.environ | {"WU_WEI_FLEET_ENABLED": flag}, capture_output=True, text=True)
...
def test_unpaid_invalid_disabled_never_execute_or_settle(monkeypatch, tmp_path):
    ...
    with patch.object(core, "_gate_inner", side_effect=AssertionError("unpaid work")), \
        ...
        assert asyncio.run(handler(Request(args | {"profiles": []}))).status_code == 400
        monkeypatch.setenv("WU_WEI_FLEET_ENABLED", "0")
...
def test_simulated_payment_delivers_report_and_replay_cannot_charge_again(monkeypatch, tmp_path):
    ...
    token = base64.b64encode(json.dumps({"x402Version": 1, "scheme": "exact", "network": "base",
        "payload": {"nonce": "wu_wei-local-fixture"}}).encode()).decode()
    with patch.object(x402_rail, "verify_and_settle", return_value={
        ...
        assert '"execution_authorized":false' in response.body.decode()
