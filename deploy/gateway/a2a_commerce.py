...
from x402_http import (
    ACQUISITION_SOURCE_HEADER, INTRO_PAYER_HEADER, INTRO_SCHEDULE,
    INTRO_SEEN_KEY, INTRO_EXEMPT_ROUTES,
...
def agent_card(public_base: str) -> dict:
    ...
    from value_decision import decision_discovery, route_value_contract
    ...
    for agent, tool in X402_HTTP_TOOLS:
        ...
        skills.append({
            ...
            "metadata": {
                ...
                "valueDecision": route_value_contract(agent, tool),
            },
        })
    return {
        ...
        "metadata": {
            "viridis.valueDecision": decision_discovery(base),
            "viridis.adoption": adoption_discovery(base),
        },
        "skills": skills,
    }
    ...
    async def send_message(request):
        ...
        if task_id:
            ...
            consumed[key] = _classified_settlement(
                ...
                surface="a2a-x402-v2",
                acquisition_source_value=task["metadata"].get(
                    "viridis.acquisition_source"))
            ...
            if final_state == "TASK_STATE_COMPLETED":
                ...
                out, delivery_receipt = _attach_paid_delivery_receipt(
                    out, consumed[key], agent, tool, public_base)
            _record_delivery_status(
                consumed[key],
                "delivered" if final_state == "TASK_STATE_COMPLETED"
                else "failed",
                delivery_receipt,
            )
        ...
        task = _payment_task(task_id, context_id, message,
                             ...
                             request.headers.get(ACQUISITION_SOURCE_HEADER))
