...
def test_regulatory_watch_contract_is_bounded_and_truthful():
    ...
    assert metadata["input_schema"]["properties"]["lookback_days"] == {
        ...
        "maximum": 365,
        "description": (
            "Symmetric window for recently effective dates behind today and "
            "effective/deadline dates ahead of today"),
    }
...
@pytest.mark.parametrize("bad_value", [0, 366, 1.5, True])
def test_regulatory_watch_refuses_invalid_window_before_quote_or_payment(
    ...
    refused = go(handler, FakeRequest(
        ...
        body={"jurisdiction": "US", "lookback_days": bad_value}))
...
@pytest.mark.parametrize("bad_topics", [
    "emissions",
    [""],
    ["x" * 101],
    [str(index) for index in range(11)],
])
def test_regulatory_watch_refuses_invalid_topics_before_quote_or_payment(
        tmp_path, monkeypatch, bad_topics):
    arm(monkeypatch)
    fake = FakeFacilitator()
    install_fake(monkeypatch, fake)
    handler, core, _ = build(
    ...
    refused = go(handler, FakeRequest(
        agent="regulatory-radar", tool="monitor_changes",
        body={"jurisdiction": "US", "topics": bad_topics,
              "lookback_days": 90}))
...
def test_X2_4_X2_5_settle_persist_serve_and_replay_refusal(
    ...
    assert delivery["feedback"]["version"] == "viridis-buyer-feedback-v1"
    assert delivery["feedback"]["endpoint"] == (
        "https://mcp.test/x402/feedback")
    assert len(delivery["feedback"]["feedback_token"]) >= 32
    assert delivery["feedback"]["exactly_once"] is True
    assert delivery["feedback"]["independently_verified"] is False
    ...
    assert len(stored["feedback_token_sha256"]) == 64
    assert stored["acquisition_source"] == "github"
    ...
    cohorts = x402_http.settlement_metrics({
        "regulatory-radar": getattr(restored, GATE_ATTR),
    })["retention_cohorts"]
    assert cohorts["by_acquisition_source"]["github"]["payer_count"] == 1
...
def test_retention_cohorts_are_aggregate_and_group_by_first_route_and_source():
    first = {
        "surface": "http-402-v2", "classification_version": 1,
        "route": "regulatory-radar/scan_regulations",
        "payer_wallet": "0xPrivateFromOutput", "self_settle": False,
        "amount_atomic": "250000", "tx_hash": "0xfirst",
    ...
    metrics = x402_http.settlement_metrics(
        {"regulatory-radar": {"consumed_x402": {
            "first": first, "repeat": repeat,
        }}},
        as_of=datetime(2026, 7, 31, tzinfo=timezone.utc),
    )
    cohorts = metrics["retention_cohorts"]
    assert cohorts["windows"]["7d"]["repeat_rate_bps"] == 10000
    assert cohorts["by_first_route"][
        "regulatory-radar/scan_regulations"]["payer_count"] == 1
    assert cohorts["by_acquisition_source"]["github"]["payer_count"] == 1
    assert "0xprivatefromoutput" not in str(cohorts).lower()
...
def test_paid_success_offers_next_paid_routes_without_auto_execution(
    ...
    assert seat == {
        ...
        "checkout_url": "https://mcp.test/seats?source=x402-success",
    ...
    retention = commerce["recommended_retention_offer"]
    assert f"{retention['agent']}/{retention['tool']}" == \
        "regulatory-radar/monitor_changes"
    assert retention["offer_type"] == "repeat_value"
    assert retention["priority"] == 1
    assert retention["auto_execute"] is False
    assert retention["buyer_authorization_required"] is True
    assert retention["quote"]["preflight_required"] is True
    assert retention["quote"]["payer_hint_authorizes_payment"] is False
    assert "dated watch" in retention["selection_reason"]
