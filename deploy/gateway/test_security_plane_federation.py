...
def _security_member() -> dict:
    return next(
        member for member in gateway.HALTED_SECURITY_MEMBERS
...
def test_sp1b_security_plane_lists_three_isolated_commercial_services():
    security = [member for member in gateway.HALTED_SECURITY_MEMBERS
...
def test_sp2_halted_security_runtime_is_not_published(tmp_path):
    ...
    assert not any(item["name"].startswith("Viridis Security") for item in directory["federated_members"])
    assert ard_response.status_code == 200
    halted_ids = {member["identifier"] for member in gateway.HALTED_SECURITY_MEMBERS}
    assert not any(item["identifier"] in halted_ids for item in ard_response.json()["entries"])
    assert all(member["status"] == "halted" for member in gateway.HALTED_SECURITY_MEMBERS)
