...
_SEAT_ACQUISITION_SOURCES = frozenset({
...
def verify_session(session_id: str, *, api_key: str | None = None,
    ...
    Returns {status:"ok", session_id, checkout_status, payment_status,
    amount_total, currency, created, expires_at, livemode} or a structured
    error envelope. Never echoes the key.
    """
    ...
    return {
        ...
        "checkout_status": resp.get("status"),
...
def list_checkout_sessions(*, created_after_epoch: int | None = None,
    ...
    Returns {status:"ok", sessions:[{session_id, checkout_status,
    payment_status, amount_total, currency, created, expires_at, livemode,
    mode}], has_more} or a structured error envelope. Never echoes the key;
    never mutates anything on Stripe.
    """
...
def list_subscription_checkout_evidence(
    *,
    ...
    """Read Viridis seat Checkout evidence for activation reconciliation.

    This internal primitive paginates a bounded Stripe list read. It returns
    only Session/subscription identifiers and lifecycle fields needed for an
    aggregate join; the admin gateway tool consumes those identifiers and
    never exposes them. It performs no provider mutation.
    """
    ...
    if (not isinstance(max_pages, int) or isinstance(max_pages, bool)
        ...
        return _err(
            "bad_max_pages", "max_pages must be an int in [1, 100]",
            field="max_pages",
        )
    key = api_key or os.environ.get("STRIPE_API_KEY")
    if not key:
        return _err(
            "no_api_key",
            "STRIPE_API_KEY not configured on the host; "
            "set it to enable reconciliation",
        )
    evidence = []
    starting_after = None
    seen_ids = set()
    for page in range(1, max_pages + 1):
        params = {"limit": 100, "created[gte]": created_after_epoch}
        ...
        try:
            ...
            return _err("stripe_error", _scrub(exc, key))
        data = response.get("data")
        if not isinstance(data, list):
            return _err(
                "stripe_error", "Stripe list response missing data")
        for session in data:
            if not isinstance(session, dict):
                return _err(
                    "stripe_error", "Stripe list contains invalid Session")
            if session.get("mode") != "subscription":
                continue
            metadata = session.get("metadata")
            if not isinstance(metadata, dict):
                continue
            catalog_sha = metadata.get("catalog_sha256")
            plan_id = metadata.get("plan_id")
            if (not isinstance(catalog_sha, str)
                    or not _SHA256_RE.fullmatch(catalog_sha)
                    or not isinstance(plan_id, str)
                    or not plan_id.strip()):
                continue
            session_id = _stripe_id(session)
            if (not session_id
                    or not _CHECKOUT_ID_RE.fullmatch(session_id)
                    or session_id in seen_ids):
                return _err(
                    "stripe_error",
                    "Stripe list contains invalid or duplicate Session id",
                )
            seen_ids.add(session_id)
            subscription_id = _stripe_id(session.get("subscription"))
            if (subscription_id is not None
                    and not _SUBSCRIPTION_ID_RE.fullmatch(subscription_id)):
                return _err(
                    "stripe_error",
                    "Stripe Session contains invalid subscription id",
                )
            ...
            if checkout_status not in {"open", "complete", "expired"}:
                return _err(
                    "stripe_error",
                    "Stripe Session contains invalid checkout status",
                )
            if payment_status not in {"paid", "unpaid", "no_payment_required"}:
                return _err(
                    "stripe_error",
                    "Stripe Session contains invalid payment status",
                )
            if (checkout_status == "complete" and payment_status == "paid"
                    and subscription_id is None):
                return _err(
                    "stripe_error",
                    "paid subscription Checkout has no subscription id",
                )
            evidence.append({
        ...
        if not data:
            return _err(
                "stripe_error", "Stripe pagination stalled on empty page")
        starting_after = _stripe_id(data[-1])
        if not starting_after:
            return _err(
                "stripe_error", "Stripe pagination cursor is invalid")
    return _err(
        "pagination_incomplete",
        "Stripe Checkout evidence exceeds the bounded page limit",
        pages=max_pages,
    )
...
def create_subscription_checkout(
    ...
    *,
    ...
    if acquisition_source not in _SEAT_ACQUISITION_SOURCES:
        return _err(
            "bad_acquisition_source",
            "acquisition_source must be a finite seat source",
    ...
    form = {
        ...
        "subscription_data[metadata][acquisition_source]": acquisition_source,
...
def verify_subscription(
    ...
    *,
    ...
    return {
        ...
        "acquisition_source": metadata.get("acquisition_source"),
