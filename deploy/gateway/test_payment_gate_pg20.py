...
Also covers PG25: health/status reads must report effective current-day zero
for an idle agent whose persisted gate state belongs to an earlier UTC day,
without mutating that state or closing its outstanding meter period.
...
def test_pg25_prior_day_status_is_truthful_and_side_effect_free(db):
    """An idle prior-day gate reports zero today while preserving raw state
    for the next real call's PG7 invoice-closing rollover."""
    _, gate, scale = build(db, free=1)
    assert call(scale, "ext:aaa111")["status"] == "ok"
    assert call(scale, "ext:aaa111")["error_type"] == "payment_required"
    raw = getattr(scale, GATE_ATTR)
    raw["day"] = "2000-01-01"
    before = {
        "day": raw["day"],
        "used": raw["used"],
        "used_by_caller": dict(raw["used_by_caller"]),
        "refused": raw["refused"],
        "refused_by_caller": dict(raw["refused_by_caller"]),
        "meter_id": raw["meter_id"],
    }

    status = gate.status()

    assert status["usage_today"]["smartscale"] == 0
    assert status["free_tier_policy"]["callers_seen_today"]["smartscale"] == 0
    counter = status["free_tier_policy"]["counter_state"]["smartscale"]
    assert counter["stored_day"] == "2000-01-01"
    assert counter["report_day"] != "2000-01-01"
    assert counter["stale"] is True
    conversion = status["conversion"]["per_agent"]["smartscale"]
    assert conversion["refusals_today"] == 0
    assert conversion["refused_callers_today"] == 0
    assert conversion["stored_counter_day"] == "2000-01-01"
    assert conversion["counter_day"] == counter["report_day"]
    assert conversion["counter_stale"] is True
    assert {
        "day": raw["day"],
        "used": raw["used"],
        "used_by_caller": dict(raw["used_by_caller"]),
...
def test_pg25_prior_day_status_stays_truthful_after_restore(db):
    """Persisted stale counters cannot reappear as 'today' after restart."""
    store, _, scale = build(db, free=1)
    assert call(scale, "ext:aaa111")["status"] == "ok"
    assert call(scale, "ext:aaa111")["error_type"] == "payment_required"
    raw = getattr(scale, GATE_ATTR)
    raw["day"] = "2000-01-01"
    store.save("smartscale", scale)

    store2 = StateStore(db)
    scale2 = SMARTSCALE.SmartScaleCore()
    meter2 = METERING.build()
    assert store2.restore("smartscale", scale2) is True
    store2.attach("smartscale", scale2)
    store2.attach("metering", meter2)
    gate2 = PaymentGate(store2, meter2, free_calls_per_day=1)
    gate2.attach("smartscale", scale2)
    ...
    assert conversion["refused_callers_today"] == 0
    assert conversion["counter_stale"] is True
    restored = getattr(scale2, GATE_ATTR)
    assert restored["day"] == "2000-01-01"
