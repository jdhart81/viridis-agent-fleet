# DEPLOY — agent-arbitration-agent

Dispute oracle: deterministic, machine-verifiable rulings closing escrow's DISPUTED loop.

## Serve
```
cd agent-arbitration-agent
...
```
Hosted remote target: `https://mcp.viridis.earth/arbitration/mcp` (streamable-http).
...
(key tools: file_case / rule / verify_ruling). Every tool returns the fleet-standard JSON envelope:
`{"status": "ok", "data": ...}` or a structured error envelope — callers
never see an exception.

## Before publish
1. `python3 deploy/mcp-publish/smoke_all.py agent-arbitration-agent`
