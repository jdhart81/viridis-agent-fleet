#!/usr/bin/env python3
"""Exact-image, copied-state rollout for the authorized Security-origin update.
...
BASE = "sha256:3201fb43b147222b0696155cc3840db9e63a0c35652387c3be304ffe4ea9e6d1"
LIVE = "viridis-fleet-gateway-1"
ROOT = Path("/root/viridis-candidates/security-origin-20260906")
SOURCE = Path("/root/viridis-fleet")
ROLLBACK = "viridis-stable:rollback-security-origin-20260906"
CANDIDATE = "viridis-stable:security-origin-20260906"
REHEARSAL = "viridis-agent-discovery-rehearsal-20260906"
PUBLIC = "https://mcp.viridisconservation.com"
LOCAL = "http://127.0.0.1:18408"
...
def request(base, path, payload=None, headers=None):
    req = urllib.request.Request(base + path, headers={"accept": "application/json",
        ...
        "x-viridis-acquisition-source": "internal", **(headers or {})},
...
def verify_surface(base, label):
    ...
    marker = {"X-Viridis-Security-Origin": security}
    status, headers, _ = request(base, "/x402/security-preflight/security_preflight", INPUTS, marker)
    assert status == 402
    encoded = next(v for k, v in headers.items() if k.lower() == "payment-required")
    security_terms = json.loads(base64.urlsafe_b64decode(encoded + "=" * (-len(encoded) % 4)))
    assert security_terms["resource"]["url"] == security + "/x402/security-preflight/security_preflight"
    assert terms["resource"]["url"] == base + "/x402/security-preflight/security_preflight"
    status, _, security_watch = request(base, WATCH_PATH, {"inputs": INPUTS}, marker)
    assert status == 200 and security_watch["quote_request"]["url"].startswith(security + "/")
    for page in ["/security-preflight", "/security-preflight/"]:
        status, headers, body = request(base, page)
        assert status == 200 and "MCP Security Preflight" in body
        assert 'rel="canonical" href="https://mcp.viridisconservation.com/security-preflight"' in body
        assert "noindex" not in headers.get("X-Robots-Tag", "").lower()
    status, _, body = request(base, "/sitemap.xml")
    assert status == 200 and body.count("<loc>") == 7
    assert "<loc>" + base + "/security-preflight</loc>" in body
    ...
    save(f"verification-{label}.json", {"status": "ok", "health": health,
         ...
         "security_quote_terms": security_terms, "security_watch": security_watch,
...
def rehearse():
    ...
    args = ["docker", "run", "-d", "--name", REHEARSAL, "--network", "viridis-fleet_default",
            "--env-file", str(env_path), "-p", "127.0.0.1:18408:8402", "-v", str(scratch) + ":/data"]
