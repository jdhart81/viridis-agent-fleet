...
def _default_transport(url: str, body: bytes, timeout_s: int) -> tuple:
    ...
    Streamable-http servers (incl. FastMCP) answer tools/call with an SSE
    stream that MAY stay open after the response event. Reading to EOF would
    block until the socket timeout, so for text/event-stream we read
    incrementally and stop at the first complete `data:` JSON-RPC message
    (bounded by V10's size cap). Plain application/json is read whole."""
    ...
    with urllib.request.urlopen(req, timeout=timeout_s) as resp:  # nosec V1-guarded
        ...
        # SSE: accumulate until a data: line parses as JSON, or cap/timeout.
        buf, total = [], 0
        for rawline in resp:
            total += len(rawline)
            if total > MAX_RESPONSE_BYTES:                        # V10
                buf.append("x" * (MAX_RESPONSE_BYTES + 1))
                break
            ...
            if s.startswith("data:"):
                ...
                try:
                    json.loads(payload)
                    break            # first complete JSON-RPC message: done
                except ValueError:
                    continue
        return (resp.status, ctype, "".join(buf))
...
class VerifiedRelayCore(AgentCore):
    ...
    KNOWN_ACTIONS = frozenset({
        "register_service", "call_verified", "get_receipt",
